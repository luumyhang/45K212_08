...
function countAllNews($Mien)
{
    global $connect;
    $sql = "SELECT count(*) AS sodong FROM news WHERE Cat_ID = ".$Mien."";
...
function GetNewsList($CurrentPage,$Mien){
    global $connect;
    $sql = "SELECT * FROM news WHERE ProductID != 0 AND Cat_ID = ".$Mien."";
    if ($CurrentPage !== 0)
    {
        $sql .= " GROUP BY ProductID LIMIT ".($CurrentPage - 1) * PAGE_SIZE.", ".PAGE_SIZE;
    }
